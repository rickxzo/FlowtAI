...
const sendOtp = async () => {
  ...
  try {
    ...
    const res = await fetch(`http://localhost:5000/send-otp?email=${encodeURIComponent(email.value)}`, {
      credentials: 'include'
    })
...
const verifyOtp = async () => {
  ...
  try {
    ...
    const res = await fetch(
      `http://localhost:5000/confirm-otp?email=${encodeURIComponent(email.value)}&otp=${otp.value}`, {
        credentials: 'include'
      }
    )
...
const login = async () => {
  ...
  try {
    ...
    const res = await fetch(
      `http://localhost:5000/signup?username=${username.value}&email=${email.value}&password=${password.value}`,
      {
